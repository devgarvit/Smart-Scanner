...
import com.eulerslab.scanner.fragment.pictureBrowserFragment;
...
import com.eulerslab.scanner.utils.picture_Adapter;
...
public class ImageDisplayActivity extends AppCompatActivity implements itemClickListener {
    ...
    public void onPicClicked(PicHolder holder, int position, ArrayList<pictureFacer> pics) {
        if (pics.size() > 0 && pics.get(0).getPicturePath().contains("PDF")) {
            Intent intent = new Intent(this, PdfViewerActivity.class);
            intent.putExtra("fileName", pics.get(0).getPicturePath());
    ...
    public ArrayList<pictureFacer> getAllImagesByFolder(String path) {
        ...
        try {
            ...
            for (int i = 0; i < pictures.length; i++) {
                String picPath = pictures[i] + "";
                String folderpaths[] = picPath.split("/");
                String picName = folderpaths[folderpaths.length - 1];
